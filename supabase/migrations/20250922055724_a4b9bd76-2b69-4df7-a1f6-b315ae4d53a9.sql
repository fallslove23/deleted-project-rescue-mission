-- Fix get_survey_detail_stats numeric parsing to use safe_numeric_convert like other analysis functions
-- This ensures averages and rating distributions are populated correctly even when values are stored as JSON/text
...
AS $$
  ...
  ordered_responses AS (
    SELECT fr.*, ROW_NUMBER() OVER (ORDER BY fr.submitted_at DESC, fr.id DESC) AS rn
    FROM filtered_responses fr
  ),
  response_total AS (
    SELECT COUNT(*)::bigint AS total_count FROM ordered_responses
  ),
  response_page AS (
    SELECT * FROM ordered_responses
  ...
  filtered_answers AS (
    SELECT qa.id, qa.question_id, qa.response_id, qa.answer_text, qa.answer_value, qa.created_at
  ...
  answer_metrics AS (
    SELECT
      ...
      fa.answer_text,
      fa.answer_value,
      -- Normalize to text and then to numeric using safe_numeric_convert (handles quotes etc.)
      public.safe_numeric_convert(
        COALESCE(NULLIF(fa.answer_value::text, ''), NULLIF(fa.answer_text, ''))
      ) AS numeric_value,
      CASE
        WHEN qb.question_type IN ('scale','rating') THEN
          CASE
            WHEN public.safe_numeric_convert(COALESCE(NULLIF(fa.answer_value::text, ''), NULLIF(fa.answer_text, ''))) IS NULL THEN NULL
            WHEN public.safe_numeric_convert(COALESCE(NULLIF(fa.answer_value::text, ''), NULLIF(fa.answer_text, ''))) <= 5
              THEN public.safe_numeric_convert(COALESCE(NULLIF(fa.answer_value::text, ''), NULLIF(fa.answer_text, ''))) * 2
            ELSE public.safe_numeric_convert(COALESCE(NULLIF(fa.answer_value::text, ''), NULLIF(fa.answer_text, '')))
          END
        ELSE NULL
  ...
  option_counts AS (
    SELECT
      question_id,
      COALESCE(jsonb_agg(
        jsonb_build_object('option', answer_text, 'count', option_count)
        ORDER BY option_count DESC, answer_text
      ), '[]'::jsonb) AS options_json
    FROM (
      SELECT am.question_id, am.answer_text, COUNT(*) AS option_count
      FROM answer_metrics am
      WHERE am.question_type IN ('multiple_choice','single_choice','dropdown','select')
        AND am.answer_text IS NOT NULL
      GROUP BY am.question_id, am.answer_text
    ) AS option_source
    GROUP BY question_id
  ),
  ordered_questions AS (
    SELECT qsr.*, COALESCE(oc.options_json, '[]'::jsonb) AS option_counts,
  ...
  text_answers_base AS (
    SELECT am.answer_id, am.question_id, am.question_text, am.satisfaction_type, am.order_index,
           am.session_id, am.section_id, am.answer_text, am.created_at
    FROM answer_metrics am
    WHERE am.question_type IN ('text','long_text','textarea','paragraph') AND am.answer_text IS NOT NULL
  ),
  ordered_text_answers AS (
    SELECT tab.*, ROW_NUMBER() OVER (ORDER BY tab.created_at DESC, tab.answer_id) AS rn
  ...
  text_page AS (
    SELECT * FROM ordered_text_answers
  ...
  text_json AS (
    ...
    ), '[]'::jsonb) AS items, MAX(tp.rn) AS last_rn
    FROM text_page tp
  ),
  summary_stats AS (
    SELECT
  ...
  SELECT
    ...
    CASE WHEN rj.last_rn IS NOT NULL AND rj.last_rn < rt.total_count THEN rj.last_rn ELSE NULL END AS response_next_cursor,
    rt.total_count AS response_total_count,
    qj.items AS question_distributions,
    CASE WHEN qj.last_rn IS NOT NULL AND qj.last_rn < qt.total_count THEN qj.last_rn ELSE NULL END AS distribution_next_cursor,
    qt.total_count AS distribution_total_count,
    tj.items AS text_answers,
    CASE WHEN tj.last_rn IS NOT NULL AND tj.last_rn < tt.total_count THEN tj.last_rn ELSE NULL END AS text_next_cursor,
...
-- Grant execute remains the same (ensure both anon and authenticated can call it)
