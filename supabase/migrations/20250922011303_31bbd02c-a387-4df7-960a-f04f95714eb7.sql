-- Drop the existing function and recreate with proper numeric handling to fix course reports
DROP FUNCTION IF EXISTS public.course_report_statistics(integer, text, integer, uuid, boolean);

-- Recreate the function with fixed numeric conversion and proper data handling
...
WITH base AS (
  SELECT
    ...
    -- Use safe_numeric_convert to handle quoted strings properly
...
GRANT EXECUTE ON FUNCTION public.course_report_statistics(integer, text, integer, uuid, boolean) TO anon;