-- Security Enhancement: Add instructor mapping and RLS policies (Fixed)
...
-- 4) RLS Helper functions (update existing ones)
...
-- Instructors can only see responses from their sessions (fixed relationship)
...
USING (
  is_instructor() AND EXISTS (
    ...
    WHERE ps.session_id = survey_responses.session_id
