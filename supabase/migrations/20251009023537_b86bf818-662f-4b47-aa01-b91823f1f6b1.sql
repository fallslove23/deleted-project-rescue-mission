-- Fix instructor count and overall satisfaction calculation in get_course_reports_working
-- Issue: instructorCount only counts instructors with responses, should count all with surveys
-- Also ensure 0 values (no data) are excluded from averages
...
BEGIN
  ...
  SELECT json_build_object(
    'summary', json_build_object(
      ...
      -- FIX: Count all instructors with surveys, not just those with responses
