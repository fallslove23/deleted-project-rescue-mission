-- Update get_course_reports_working function to use safe_numeric_convert for satisfaction calculations
CREATE OR REPLACE FUNCTION public.get_course_reports_working(p_year integer, p_course_name text DEFAULT NULL::text, p_round integer DEFAULT NULL::integer, p_instructor_id uuid DEFAULT NULL::uuid, p_include_test boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
...
BEGIN
  ...
  instructor_pre AS (
    SELECT
      ...
      COALESCE(SUM(response_count), 0)::int AS responses,
      CASE WHEN COALESCE(SUM(response_count), 0) > 0 THEN ROUND(
        SUM(COALESCE(
          COALESCE(avg_instructor_satisfaction, avg_course_satisfaction, avg_operation_satisfaction),
          0
        ) * response_count) / NULLIF(SUM(response_count), 0), 2
