-- 1) Create survey analysis comments table with RLS and triggers
...
DO $$
BEGIN
  -- View: admins, operators, directors, and instructors assigned to the survey
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname='public' AND tablename='survey_analysis_comments' AND policyname='View comments') THEN
    EXECUTE $$
      CREATE POLICY "View comments" ON public.survey_analysis_comments
      FOR SELECT
      USING (
  ...
  -- Insert: allowed for admins/operators/directors/instructors; must be author
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname='public' AND tablename='survey_analysis_comments' AND policyname='Insert comments') THEN
    EXECUTE $$
      CREATE POLICY "Insert comments" ON public.survey_analysis_comments
      FOR INSERT
      WITH CHECK (
        (is_admin() OR is_operator() OR is_director() OR is_instructor())
        AND author_id = auth.uid()
      )
    $$;
  END IF;

  -- Update: author or admin/operator/director can update
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname='public' AND tablename='survey_analysis_comments' AND policyname='Update own or admin') THEN
    EXECUTE $$
      CREATE POLICY "Update own or admin" ON public.survey_analysis_comments
      FOR UPDATE
      USING (author_id = auth.uid() OR is_admin() OR is_operator() OR is_director())
      WITH CHECK (author_id = auth.uid() OR is_admin() OR is_operator() OR is_director())
    $$;
  END IF;

  -- Delete: author or admin/operator/director
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname='public' AND tablename='survey_analysis_comments' AND policyname='Delete own or admin') THEN
    EXECUTE $$
      CREATE POLICY "Delete own or admin" ON public.survey_analysis_comments
      FOR DELETE
      USING (author_id = auth.uid() OR is_admin() OR is_operator() OR is_director())
    $$;
  END IF;
...
-- 2) Ensure primary admin role for sethetrend87@osstem.com (idempotent)
DO $$
DECLARE
  admin_uid uuid;
BEGIN
