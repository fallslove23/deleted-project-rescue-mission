-- Update get_course_reports_working to map instructors via survey_sessions and constrain responses by session
...
BEGIN
  WITH survey_base AS (
    SELECT DISTINCT
      ...
      s.education_round,
      s.course_name,
      -- Prefer session-level instructor mapping when available
      ss.id AS session_id,
  ...
  response_stats AS (
    ...
    GROUP BY sb.survey_id, sb.session_id, sb.education_year, sb.education_round, sb.course_name, sb.instructor_id, sb.instructor_name
  ),
  summary_calc AS (
    SELECT
