-- Fix the safe_numeric_convert function to handle double-quoted strings and improve course report function
DROP FUNCTION IF EXISTS public.safe_numeric_convert(text);
...
BEGIN
  ...
  input_text := TRIM(REGEXP_REPLACE(input_text, '[''"]', '', 'g'));
...
-- Also create a simple function to handle the specific issue with the data
CREATE OR REPLACE FUNCTION public.course_report_statistics_v2(
  p_year integer,
