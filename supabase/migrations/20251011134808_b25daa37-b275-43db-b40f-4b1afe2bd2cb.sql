-- Fix get_course_reports_working: replace MIN(uuid) with ORDER BY ... LIMIT 1 to avoid 42883 error
  ...
  response_stats AS (
    ...
    LEFT JOIN public.survey_responses sr 
      ...
      AND (
        ...
        OR 
        -- 2) if a response has no session, attach it to the first session id to avoid double counting (no min(uuid))
