-- Fix instructor count calculation and better handle null instructor_id cases
...
BEGIN
  WITH survey_base AS (
    SELECT DISTINCT
      ...
      s.course_name,
      s.instructor_id,
      COALESCE(i.name, '강사 미지정') AS instructor_name,
      s.is_test
  ...
  instructor_pre AS (
    ...
    FROM response_stats
    -- Include both assigned instructors and unassigned surveys (as '강사 미지정')
  ...
  instructor_json AS (
    SELECT COALESCE(
      json_agg(
        ...
        ORDER BY 
          -- Put assigned instructors first, then unassigned
          CASE WHEN instructor_id IS NOT NULL THEN 0 ELSE 1 END,
          instructor_name NULLS LAST
      ), '[]'::json
    ) AS instructor_stats
    FROM instructor_pre
  ),
  ...
  available_instructors_pre AS (
    ...
    FROM response_stats
    WHERE instructor_id IS NOT NULL  -- Only show actual instructors in available list
  ),
  ...
  SELECT json_build_object(
    'summary', json_build_object(
      ...
      -- Fixed instructor count to include all instructors (assigned + unassigned as separate entries)
      'instructorCount', (SELECT COUNT(*) FROM instructor_pre)
    ),
