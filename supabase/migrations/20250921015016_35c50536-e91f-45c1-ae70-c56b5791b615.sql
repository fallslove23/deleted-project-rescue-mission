...
DROP MATERIALIZED VIEW IF EXISTS public.survey_cumulative_stats;

-- Create survey_cumulative_stats as a regular view (not materialized) to support RLS
CREATE OR REPLACE VIEW public.survey_cumulative_stats AS
SELECT 
    s.id as survey_id,
    s.title,
    s.education_year,
...
ALTER VIEW public.survey_cumulative_stats SET (security_barrier = true);
