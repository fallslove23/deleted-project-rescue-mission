-- Patch get_course_reports_working to include sessionless responses in aggregation
...
BEGIN
  ...
  response_stats AS (
    ...
    LEFT JOIN public.survey_responses sr 
      ...
      AND (
        -- 1) session row aggregates that session's responses
        (sb.session_id IS NOT NULL AND sr.session_id = sb.session_id)
        OR 
        -- 2) if a response has no session, attach it to the smallest session_id to avoid double counting
        (sb.session_id IS NOT NULL AND sr.session_id IS NULL AND sb.session_id = (
          SELECT MIN(ss2.id) FROM public.survey_sessions ss2 WHERE ss2.survey_id = sb.survey_id
        ))
        OR
        -- 3) surveys without sessions aggregate all responses
