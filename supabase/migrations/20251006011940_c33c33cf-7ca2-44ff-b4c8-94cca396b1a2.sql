-- 1) Add helper: normalize_course_name (immutable)
CREATE OR REPLACE FUNCTION public.normalize_course_name(input_name text)
RETURNS text
LANGUAGE sql
IMMUTABLE
      ...
      regexp_replace(
        regexp_replace(
          regexp_replace(
            regexp_replace(
              regexp_replace(
                regexp_replace(input_name, '\\((?:홀수조|짝수조)\\)', '', 'g'),
              '\\b\\d{1,2}/\\d{1,2}조\\b', '', 'g'),
            '(\\d+차-\\d+일차)\\s+\\d{1,2}조', '\\1', 'g'),
          '(\\d+차-\\d+일차)\\s+(?:홀수조|짝수조)', '\\1', 'g'),
        '\\b\\d{1,2}\\s*(?:조|반)\\b', '', 'g'),
      '(?:홀수조|짝수조)-', '', 'g')
    ),
    ''
...
-- 2) Update RPC: get_course_reports_working to use normalized matching and produce normalized/aggregated course options
...
BEGIN
  WITH survey_base AS (
    ...
    WHERE s.education_year = p_year
      ...
      AND (
        ...
        OR public.normalize_course_name(s.course_name) = p_course_normalized
      )
  ...
  SELECT json_build_object(
    'summary', json_build_object(
      ...
      'instructorCount', (SELECT COUNT(*) FROM instructor_pre WHERE responses > 0)
    ),
