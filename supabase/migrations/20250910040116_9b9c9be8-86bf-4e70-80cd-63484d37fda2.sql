...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Public: view active or public surveys" ON public.surveys FOR SELECT USING (status = ANY (ARRAY[''active''::text, ''public''::text]) AND (start_date IS NULL OR now() >= start_date) AND (end_date IS NULL OR now() <= end_date));';
  END IF;
END $$;