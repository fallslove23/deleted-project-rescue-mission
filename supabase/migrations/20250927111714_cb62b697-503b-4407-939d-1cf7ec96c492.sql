-- Fix instructor mapping by using survey_instructors table and correct satisfaction_type labels
...
BEGIN
  WITH survey_base AS (
    SELECT DISTINCT
      ...
      s.education_round,
      s.course_name,
      -- Use instructor from surveys table first, then from survey_instructors mapping table
  ...
  response_stats AS (
    SELECT 
      ...
      -- 'course' satisfaction_type is actually '과목(lecture)' satisfaction, not '과정(course)' satisfaction
      AVG(CASE 
  ...
  instructor_pre AS (
    ...
    FROM response_stats
    -- Only include entries with actual instructor assignments (exclude 강사 미지정)
  ...
  SELECT json_build_object(
    'summary', json_build_object(
      ...
      -- Correct instructor count using proper mapping
