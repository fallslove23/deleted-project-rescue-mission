...
const SurveyBuilder = () => {
  ...
  const generateSurveyTitle = () => {
    const course = allCourses.find(c => c.id === courseForm.selectedCourseId);
    const courseTitle = course?.title || '';
    
    if (surveyForm.course_name && surveyForm.education_year && surveyForm.education_round) {
      return `[${surveyForm.education_year}년 ${surveyForm.education_round}차] ${surveyForm.course_name} - ${courseTitle}`;
    } else if (surveyForm.course_name) {
      return `${surveyForm.course_name} - ${courseTitle}`;
    } else {
      return courseTitle;
    }
  };
  ...
  const handleUpdateSurveyInfo = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const convertToUTC = (datetimeLocal: string) => {
        ...
        // Create date from local datetime input and convert to UTC
        const localDate = new Date(datetimeLocal);
        return localDate.toISOString();
      };

      // 제목 자동 생성
      const autoGeneratedTitle = generateSurveyTitle();
      const finalTitle = autoGeneratedTitle || surveyForm.title;
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          start_date: convertToUTC(surveyForm.start_date),
          end_date: convertToUTC(surveyForm.end_date),
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          title: autoGeneratedTitle || survey?.title
        })
        .eq('id', surveyId);
